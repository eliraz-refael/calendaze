defmodule CalenDaze.BusinessAccounts do
  @moduledoc """
  The BusinessAccounts context.
  """

  import Ecto.Query, warn: false
  alias CalenDaze.Repo

  alias CalenDaze.BusinessAccounts.{Business, BusinessToken, BusinessNotifier}
  ...
  def change_business_registration(%Business{} = business, attrs \\ %{}) do
    Business.registration_changeset(business, attrs, hash_password: false, validate_email: false)
  end
  ...
  def change_business_email(business, attrs \\ %{}) do
    Business.email_changeset(business, attrs, validate_email: false)
  end
  ...
  defp business_email_multi(business, email, context) do
    changeset =
      business
      |> Business.email_changeset(%{email: email})
      |> Business.confirm_changeset()

    Ecto.Multi.new()
    |> Ecto.Multi.update(:business, changeset)
    |> Ecto.Multi.delete_all(:tokens, BusinessToken.by_business_and_contexts_query(business, [context]))
  end
  ...
  ## Examples
      ...
      iex> deliver_business_update_email_instructions(business, current_email, &url(~p"/businesses/settings/confirm_email/#{&1})")
      {:ok, %{to: ..., body: ...}}

  """
  def deliver_business_update_email_instructions(%Business{} = business, current_email, update_email_url_fun)
  ...
  def update_business_password(business, password, attrs) do
    ...
    Ecto.Multi.new()
    |> Ecto.Multi.update(:business, changeset)
    |> Ecto.Multi.delete_all(:tokens, BusinessToken.by_business_and_contexts_query(business, :all))
    |> Repo.transaction()
    |> case do
  ...
  """
  def delete_business_session_token(token) do
    Repo.delete_all(BusinessToken.by_token_and_context_query(token, "session"))
    :ok
  end
  ...
  ## Examples
      ...
      iex> deliver_business_confirmation_instructions(business, &url(~p"/businesses/confirm/#{&1}"))
      {:ok, %{to: ..., body: ...}}

      iex> deliver_business_confirmation_instructions(confirmed_business, &url(~p"/businesses/confirm/#{&1}"))
  ...
  defp confirm_business_multi(business) do
    ...
    |> Ecto.Multi.delete_all(:tokens, BusinessToken.by_business_and_contexts_query(business, ["confirm"]))
  end
  ...
  ## Examples
      ...
      iex> deliver_business_reset_password_instructions(business, &url(~p"/businesses/reset_password/#{&1}"))
      {:ok, %{to: ..., body: ...}}
