defmodule CalenDaze.BusinessAccountsTest do
  use CalenDaze.DataCase

  alias CalenDaze.BusinessAccounts

  import CalenDaze.BusinessAccountsFixtures
  alias CalenDaze.BusinessAccounts.{Business, BusinessToken}

  describe "get_business_by_email/1" do
    test "does not return the business if the email does not exist" do
      refute BusinessAccounts.get_business_by_email("unknown@example.com")
    end

    test "returns the business if the email exists" do
      %{id: id} = business = business_fixture()
      assert %Business{id: ^id} = BusinessAccounts.get_business_by_email(business.email)
    end
  end

  describe "get_business_by_email_and_password/2" do
    test "does not return the business if the email does not exist" do
      refute BusinessAccounts.get_business_by_email_and_password("unknown@example.com", "hello world!")
    end

    test "does not return the business if the password is not valid" do
      business = business_fixture()
      refute BusinessAccounts.get_business_by_email_and_password(business.email, "invalid")
    end
    ...
    test "returns the business if the email and password are valid" do
      ...
      assert %Business{id: ^id} =
               BusinessAccounts.get_business_by_email_and_password(business.email, valid_business_password())
    end
  end
  ...
  describe "get_business!/1" do
    test "raises if id is invalid" do
      assert_raise Ecto.NoResultsError, fn ->
        BusinessAccounts.get_business!("11111111-1111-1111-1111-111111111111")
      end
    end

    test "returns the business with the given id" do
      %{id: id} = business = business_fixture()
      assert %Business{id: ^id} = BusinessAccounts.get_business!(business.id)
    end
  end

  describe "register_business/1" do
    test "requires email and password to be set" do
      {:error, changeset} = BusinessAccounts.register_business(%{})
    ...
    test "validates email and password when given" do
      {:error, changeset} = BusinessAccounts.register_business(%{email: "not valid", password: "not valid"})
    ...
    test "validates maximum values for email and password for security" do
      ...
      {:error, changeset} = BusinessAccounts.register_business(%{email: too_long, password: too_long})
    ...
    test "validates email uniqueness" do
      ...
      {:error, changeset} = BusinessAccounts.register_business(%{email: email})
      assert "has already been taken" in errors_on(changeset).email

      # Now try with the upper cased email too, to check that email case is ignored.
      {:error, changeset} = BusinessAccounts.register_business(%{email: String.upcase(email)})
      assert "has already been taken" in errors_on(changeset).email
    end

    test "registers businesses with a hashed password" do
      email = unique_business_email()
      {:ok, business} = BusinessAccounts.register_business(valid_business_attributes(email: email))
  ...
  describe "change_business_registration/2" do
    test "returns a changeset" do
      assert %Ecto.Changeset{} = changeset = BusinessAccounts.change_business_registration(%Business{})
    ...
    test "allows fields to be set" do
      ...
      changeset =
        BusinessAccounts.change_business_registration(
          %Business{},
  ...
  describe "change_business_email/2" do
    test "returns a business changeset" do
      assert %Ecto.Changeset{} = changeset = BusinessAccounts.change_business_email(%Business{})
  ...
  describe "apply_business_email/3" do
    ...
    test "requires email to change", %{business: business} do
      {:error, changeset} = BusinessAccounts.apply_business_email(business, valid_business_password(), %{})
      assert %{email: ["did not change"]} = errors_on(changeset)
    end

    test "validates email", %{business: business} do
      {:error, changeset} =
        BusinessAccounts.apply_business_email(business, valid_business_password(), %{email: "not valid"})
    ...
    test "validates maximum value for email for security", %{business: business} do
      ...
      {:error, changeset} =
        BusinessAccounts.apply_business_email(business, valid_business_password(), %{email: too_long})
    ...
    test "validates email uniqueness", %{business: business} do
      ...
      {:error, changeset} = BusinessAccounts.apply_business_email(business, password, %{email: email})
    ...
    test "validates current password", %{business: business} do
      {:error, changeset} =
        BusinessAccounts.apply_business_email(business, "invalid", %{email: unique_business_email()})
    ...
    test "applies the email without persisting it", %{business: business} do
      email = unique_business_email()
      {:ok, business} = BusinessAccounts.apply_business_email(business, valid_business_password(), %{email: email})
      assert business.email == email
      assert BusinessAccounts.get_business!(business.id).email != email
    end
  end

  describe "deliver_business_update_email_instructions/3" do
    setup do
      %{business: business_fixture()}
    end
    ...
    test "sends token through notification", %{business: business} do
      token =
        extract_business_token(fn url ->
          BusinessAccounts.deliver_business_update_email_instructions(business, "current@example.com", url)
        end)
  ...
  describe "update_business_email/2" do
    ...
    test "updates the email with a valid token", %{business: business, token: token, email: email} do
      assert BusinessAccounts.update_business_email(business, token) == :ok
    ...
    test "does not update email with invalid token", %{business: business} do
      assert BusinessAccounts.update_business_email(business, "oops") == :error
    ...
    test "does not update email if business email changed", %{business: business, token: token} do
      assert BusinessAccounts.update_business_email(%{business | email: "current@example.com"}, token) == :error
      assert Repo.get!(Business, business.id).email == business.email
      assert Repo.get_by(BusinessToken, business_id: business.id)
    end
    ...
    test "does not update email if token expired", %{business: business, token: token} do
      ...
      assert BusinessAccounts.update_business_email(business, token) == :error
      assert Repo.get!(Business, business.id).email == business.email
      assert Repo.get_by(BusinessToken, business_id: business.id)
    end
  end
  ...
  describe "change_business_password/2" do
    test "returns a business changeset" do
      assert %Ecto.Changeset{} = changeset = BusinessAccounts.change_business_password(%Business{})
  ...
  describe "update_business_password/3" do
    ...
    test "validates password", %{business: business} do
      {:error, changeset} =
        BusinessAccounts.update_business_password(business, valid_business_password(), %{
          password: "not valid",
    ...
    test "validates maximum values for password for security", %{business: business} do
      ...
      {:error, changeset} =
        BusinessAccounts.update_business_password(business, valid_business_password(), %{password: too_long})
    ...
    test "validates current password", %{business: business} do
      {:error, changeset} =
        BusinessAccounts.update_business_password(business, "invalid", %{password: valid_business_password()})
    ...
    test "updates the password", %{business: business} do
      ...
      assert is_nil(business.password)
      assert BusinessAccounts.get_business_by_email_and_password(business.email, "new valid password")
    end
  ...
  describe "generate_business_session_token/1" do
    ...
    test "generates a token", %{business: business} do
      token = BusinessAccounts.generate_business_session_token(business)
  ...
  describe "get_business_by_session_token/1" do
    ...
    test "returns business by token", %{business: business, token: token} do
      assert session_business = BusinessAccounts.get_business_by_session_token(token)
      assert session_business.id == business.id
    end

    test "does not return business for invalid token" do
      refute BusinessAccounts.get_business_by_session_token("oops")
    end

    test "does not return business for expired token", %{token: token} do
      {1, nil} = Repo.update_all(BusinessToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute BusinessAccounts.get_business_by_session_token(token)
    end
  end

  describe "delete_business_session_token/1" do
    test "deletes the token" do
      business = business_fixture()
      token = BusinessAccounts.generate_business_session_token(business)
      assert BusinessAccounts.delete_business_session_token(token) == :ok
      refute BusinessAccounts.get_business_by_session_token(token)
    end
  end
  ...
  describe "confirm_business/1" do
    ...
    test "confirms the email with a valid token", %{business: business, token: token} do
      assert {:ok, confirmed_business} = BusinessAccounts.confirm_business(token)
    ...
    test "does not confirm with invalid token", %{business: business} do
      assert BusinessAccounts.confirm_business("oops") == :error
    ...
    test "does not confirm email if token expired", %{business: business, token: token} do
      {1, nil} = Repo.update_all(BusinessToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      assert BusinessAccounts.confirm_business(token) == :error
      refute Repo.get!(Business, business.id).confirmed_at
      assert Repo.get_by(BusinessToken, business_id: business.id)
    end
  end
  ...
  describe "deliver_business_reset_password_instructions/2" do
    ...
    test "sends token through notification", %{business: business} do
      token =
        extract_business_token(fn url ->
          BusinessAccounts.deliver_business_reset_password_instructions(business, url)
        end)
  ...
  describe "get_business_by_reset_password_token/1" do
    ...
    test "returns the business with valid token", %{business: %{id: id}, token: token} do
      assert %Business{id: ^id} = BusinessAccounts.get_business_by_reset_password_token(token)
      assert Repo.get_by(BusinessToken, business_id: id)
    end

    test "does not return the business with invalid token", %{business: business} do
      refute BusinessAccounts.get_business_by_reset_password_token("oops")
      assert Repo.get_by(BusinessToken, business_id: business.id)
    end

    test "does not return the business if token expired", %{business: business, token: token} do
      {1, nil} = Repo.update_all(BusinessToken, set: [inserted_at: ~N[2020-01-01 00:00:00]])
      refute BusinessAccounts.get_business_by_reset_password_token(token)
      assert Repo.get_by(BusinessToken, business_id: business.id)
    end
  end
  ...
  describe "reset_business_password/2" do
    ...
    test "validates password", %{business: business} do
      {:error, changeset} =
        BusinessAccounts.reset_business_password(business, %{
          password: "not valid",
          password_confirmation: "another"
        })
    ...
    test "validates maximum values for password for security", %{business: business} do
      too_long = String.duplicate("db", 100)
      {:error, changeset} = BusinessAccounts.reset_business_password(business, %{password: too_long})
      assert "should be at most 72 character(s)" in errors_on(changeset).password
    end

    test "updates the password", %{business: business} do
      {:ok, updated_business} = BusinessAccounts.reset_business_password(business, %{password: "new valid password"})
    ...
    test "deletes all tokens for the given business", %{business: business} do
      _ = BusinessAccounts.generate_business_session_token(business)
      {:ok, _} = BusinessAccounts.reset_business_password(business, %{password: "new valid password"})
      refute Repo.get_by(BusinessToken, business_id: business.id)
    end
  end

  describe "inspect/2 for the Business module" do
